...
>**您当前访问的是 `myself` 分支，该分支的代码基本为本人手工编写，将用于课程设计考核。代码文件位于`/src`目录下。**

本课设共包含两个分支：

- **`myself` 分支**：基本手工 coding，搭配AI辅助纠错与自动补全，是**考核主体**。
- **`main` 分支**：因时间有限，`myself` 分支没有完全实现预期目标，但是`myself` 的底层代码已基本完善。`main` 中的`blockyProg` 库有大部分业务代码由 AI 生成。该分支仅用于验证自制库的通用性与完善性（证明 AI 能够掌握并运用该库），**并非为课程设计考核而设**。
...
![myself分支主机展示](./doc/myself分支主机展示.jpg)
## 需求分析
...
使用git进行版本控制，追溯开发历史。
采用断点运行调试与串口调试两种方式进行调试，并搭配AI Agent进行辅助。
## 总体设计<!-- 总体功能及模块设计，可用图示 -->
...
4. **可拓展性强**：所写库文件可以快速移植，且能够继续发展以匹配更多适用场景。
5. **git commit记录齐全**： 便于后续问题追踪和修复。

### 缺点
1. **功能单一**：目前仅支持数字输出和模拟输出，缺乏输入模块（如读取传感器数据）、逻辑控制模块（如循环、判断）和延时模块。（这一点可以通过AI辅助编写实现，详情参考main分支，该分支基于自制的UI,BCP,Touch库使用AI编程生成了带有逻辑控制的积木块，功能更为细化）
2. **UI 性能瓶颈**：在积木数量较多时，滑动和刷新帧率下降，缺乏双缓冲机制优化。
3. **通信可靠性**：ESP-NOW 为无连接协议，缺乏 ACK 重传机制，在网络干扰较强时可能出现指令丢失，类似UDP。不过由于发包频率足够高，重传并不是很必要。
4. **git使用不规范**：虽然使用了git，但是commit message 的格式和风格不符合规范，不易于快速浏览。
5. **面向对象掌握不熟练**：虽然本课设运用了基类，继承，多态，封装，但是封装不彻底，很多类成员仍然是public类型，接口过于开放，不够智能，没有使用函数间接访问成员变量，导致代码调用不规范，容易出现兼容性问题，没有完全纠正书写C语言时留下的坏习惯。
