...
## ⚠️重要提示 ⚠️
本课设分为两个分支，其中**myself分支**基本为自己**手工coding**，而**main分支**下的**blockyProg库**有大部分业务代码**完全由AI编写**，该分支只是为了**完善项目本身**，**不必纳入课程设计的考核**。
## 需求分析
...
4. **vector 容器管理与删除一致性问题（核心）**：
   ...
   - **额外实践**：
     - `uiActivity` 析构中使用“从尾部反复 delete”清空 `uiElementsPool`，可避免迭代器失效。
     - 元素析构负责把自己从容器摘除，形成“谁创建、谁释放、释放即脱链”的一致生命周期。
...
6. **ESP-NOW 配对与双向链路问题**：
   ...
   - **根因**：从机收到握手后，未把主机 MAC 写入 peer 列表，导致回包失败。
   - **修复**：在从机接收握手包（Type 0x00）后立刻 `esp_now_add_peer`；并统一使用 6 字节二进制 MAC，不使用字符串 MAC。
...
7. **触摸漂移与按键误触**：
   ...
   - **修复**：
     - 在 `Touch.cpp` 通过 `TFT_TOUCH_MINX/MAXX` 与 `TFT_TOUCH_MINY/MAXY` 多轮标定，超范围直接判定未触摸。
...
但是需要提出的是，在本次课设的完成中，AI的辅助起到了至关重要的作用，在此我有必要特地开一个副标题着重介绍。当下大模型的编程能力指数上涨，使得AI的辅助成为不可缺少的利器。不论是真正运用到工程实践中（比如我参加的电赛，RoboMaster等比赛），还是用于辅助学业，使用AI编程、差错、甚至直接用于教学的能力不容小觑。老师在课设的介绍课上提到不反对使用AI，我深有感悟。

虽然在课程设计这门课上主要需要体现的是学生自主的编程能力与对C++的掌握能力，但是要想运用到实际当中，不仅要有扎实的基本功，更要有高效的生产力。据我所知，身边的很多同学在使用AI进行辅助编程的过程中，还在使用网页端大语言模型的对话窗口，靠手动的复制粘贴与询问报错来实现AI的辅助，不可否认的是他们确实体验到了AI的便利，但是他们却不了解更成熟的Agent，以及那些专为coding优化的模型，以至于不能将AI的能力发挥到极致。

其实肉眼可见的是这篇课程设计报告也运用了AI辅助，我把和AI聊天的历史记录，尤其是询问那些报错原因，特别是运行时错误的记录，打包输入给了AI，并且通过agent让AI直接阅读我的git的commit记录，让AI帮我细化了**系统调试**章节的内容，并让AI帮助我润色。
...
### 关于工程经验
使用git用于版本控制是一个好的工程实践，这不仅有利于回溯历史，查询记录，更有利于记录工作流，以及提高团队合作效率。虽然本次课设并不是小组完成，但是看到部分同学组队完成课设，却不了解版本控制，完全靠打包压缩同步进度，个人认为不是一个好办法。
